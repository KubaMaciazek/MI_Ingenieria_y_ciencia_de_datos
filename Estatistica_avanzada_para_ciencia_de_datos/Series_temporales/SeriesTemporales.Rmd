...
# Read from an URL
# Skip 1 line - comment
data.ts.k1 <- scan("ts_k1.dat", skip = 1)
head(data.ts.k1)
```
...
```{r}
library(readr)
temperaturas <- read_csv("./temp.csv")
...
lynx <- read.csv(file = "annual-number-of-lynx-trapped-ma.csv", header = TRUE, quote = '\"')
lynx.ts <- ts(lynx$Lynx, frequency = 1, start = c(1821))
electricity <- read.csv(file = "monthly-electricity-production-i.csv", header = TRUE, quote = '\"')
